...
const JiomartLoginError = async () => {
  ...
  while (attempts < MAX_RETRIES) {
    try {
      attempts++;
      console.log(`Attempt ${attempts} to process Jiomart Login Error stats...`);
      ...
      if (validOrgQueryRows.length === 0) {
        console.log('No valid orgIds found for Jiomart Login Error.');
        return;
      }

      const orgIds = validOrgQueryRows.map(row => row.org_id);

      // Step 2: Login Login Error Query
      const JiomartLoginErrorQuery = `
        SELECT orgid, channel, report_type, status ,message
      FROM fs_upload
      WHERE created_at > (CURRENT_DATE - INTERVAL '1 day') + INTERVAL '14:30'
        AND orgid = ANY($1)
        AND channel = 'Jiomart'
        AND report_type = 'Automation'
        AND message like '%Login failed%'
        
       
      `;

      const JiomartLoginErrorQueryResult = await client.query(JiomartLoginErrorQuery, [orgIds]);

      // Log results to the terminal
      console.log(`Jiomart Login Error`);
      console.table(JiomartLoginErrorQueryResult.rows.map(row => ({
        OrgId: row.orgid,
        Channel: row.channel,
        // Message: row.message
      })));
...
module.exports = JiomartLoginError;
