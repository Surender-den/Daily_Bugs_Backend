...
const ShiprocketBankLoginError = async () => {
  let client = null;
  let attempts = 0;
  ...
  while (attempts < MAX_RETRIES) {
    try {
      attempts++;
      console.log(`Attempt ${attempts} to process Shiprocket Bank Login Error stats...`);
      ...
      const validOrgQuery = `
        ...
        WHERE org.channel = 'Shiprocketbank'
      ...
      if (validOrgQueryRows.length === 0) {
        console.log('No valid orgIds found for Shiprocket Bank Login Error.');
        return;
      }
      ...
      const ShiprocketBankLoginErrorQuery = `
        ...
        WHERE 
          ...
          AND createdby='cron' 
          AND orgid = ANY($1)
          AND (sources->'8'->'0'->>'name' = 'SHIPROCKET_SETTLEMENT_AND_BANK_STATEMENT')
          AND (sources->'8'->'0'->>'message' = 'Authentication failed')
          ORDER BY createdAt ASC;
      `;

      const ShiprocketBankLoginQueryResult = await client.query(ShiprocketBankLoginErrorQuery, [orgIds]);

      // Log results to the terminal
      console.log(`Shiprocket Bank Login Error`);
      console.table(ShiprocketBankLoginQueryResult.rows.map(row => ({
        OrgId: row.orgid,
        Channel: 'ShiprocketBank'
      })));
...
module.exports = ShiprocketBankLoginError;
