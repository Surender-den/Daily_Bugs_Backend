...
const DelhiveryBankLoginError = async () => {
  let client = null;
  let attempts = 0;
  ...
  while (attempts < MAX_RETRIES) {
    try {
      attempts++;
      console.log(`Attempt ${attempts} to process Delhivery Bank Login Error stats...`);
      ...
      if (validOrgQueryRows.length === 0) {
        console.log('No valid orgIds found for Delhivery Bank Login Error.');
        return;
      }
      ...
      const DelhiveryBankLoginErrorQuery = `
        ...
        WHERE 
          ...
          AND createdby='cron' 
          AND orgid = ANY($1)
          AND (sources->'3'->'0'->>'name' = 'DELHIVERY_BANK')
          AND (sources->'3'->>'message' like '%Invalid username or password.%')
          
        ORDER BY createdAt ASC;
      `;

      const DelhiveryBankLoginQueryResult = await client.query(DelhiveryBankLoginErrorQuery, [orgIds]);

      // Log results to the terminal
      console.log(`Delhivery Bank Login Error`);
      console.table(DelhiveryBankLoginQueryResult.rows.map(row => ({
        OrgId: row.orgid,
        Channel: 'Delhivery Bank'
      })));
...
module.exports = DelhiveryBankLoginError;
