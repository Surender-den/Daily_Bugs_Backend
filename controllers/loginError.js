...
const LoginError = async () => {
  ...
  while (attempts < MAX_RETRIES) {
    try {
      attempts++;
      console.log(`Attempt ${attempts} to process Login Error stats...`);
      ...
      const loginErrorQueryResult = await client.query(loginErrorQuery, [orgIds]);

      // Log results to the terminal
      console.log(`Login Error`);
      console.table(loginErrorQueryResult.rows.map(row => ({
        OrgId: row.orgid,
        Channel: row.channel
      })));
