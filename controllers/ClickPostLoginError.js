...
const ClickPostLoginError = async () => {
  let client = null;
  let attempts = 0;
  ...
  while (attempts < MAX_RETRIES) {
    try {
      attempts++;
      console.log(`Attempt ${attempts} to process Click Post Login Error stats...`);
      ...
      if (validOrgQueryRows.length === 0) {
        console.log('No valid orgIds found for Click Post Login Error.');
        return;
      }
      ...
      const ClickPostLoginErrorQuery = `
        ...
        WHERE 
          ...
          AND createdby='cron' 
          AND orgid = ANY($1)
    AND (sources->'6'->'0'->>'name' = 'CLICK_POST')
    AND (sources->'6'->>'message' like '%Login failed%')
          AND  status  in ('FAILED')
        ORDER BY createdAt ASC;
      `;

      const ClickPostLoginErrorQueryResult = await client.query(ClickPostLoginErrorQuery, [orgIds]);

      // Log results to the terminal
      console.log(`Click Post Login Error`);
      console.table(ClickPostLoginErrorQueryResult.rows.map(row => ({
        OrgId: row.orgid,
        Channel:'Click Post'
      })));
...
module.exports = ClickPostLoginError;
