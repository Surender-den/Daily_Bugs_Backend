...
const RazorpayLoginError = async () => {
  let client = null;
  let attempts = 0;
  ...
  while (attempts < MAX_RETRIES) {
    try {
      attempts++;
      console.log(`Attempt ${attempts} to process Razorpay Login Error stats...`);
      ...
      if (validOrgQueryRows.length === 0) {
        console.log('No valid orgIds found for Razorpay Login Error.');
        return;
      }

      const orgIds = validOrgQueryRows.map(row => row.org_id);

      // Step 2: Razorpay Login Error Query
      const RazorpayLoginErrorQuery = `
        ...
        WHERE 
          ...
          AND createdby='cron' 
          AND orgid = ANY($1)
    AND (sources->'5'->'0'->>'name' = 'RAYZORPAY_BANK')
    AND (sources->'5'->'0'->>'message' like '%Authentication failed%')
          AND  status  in ('FAILED')
        ORDER BY createdAt ASC;
      `;

      const RazorpayLoginErrorQueryResult = await client.query(RazorpayLoginErrorQuery, [orgIds]);

      // Log results to the terminal
      console.log(`Razorpay Login Error`);
      console.table(RazorpayLoginErrorQueryResult.rows.map(row => ({
        OrgId: row.orgid,
        Channel:'Razorpay'
      })));
...
module.exports = RazorpayLoginError;
