...
const invalidCookieError = async () => {
  let client = null;
  let attempts = 0;
  ...
  while (attempts < MAX_RETRIES) {
    try {
      attempts++;
      console.log(`Attempt ${attempts} to process Invalid Cookie Error stats...`);
      ...
      if (validOrgQueryRows.length === 0) {
        console.log('No valid orgIds found for invalidCookie Error.');
        return;
      }
      ...
      const invalidCookieQuery = `
        ...
        WHERE 
          ...
          AND createdby='cron' 
          AND orgid = ANY($1)
          AND message::text LIKE '%Invalid cookie%'
        ORDER BY createdAt ASC;
      `;

      const invalidCookieQueryResult = await client.query(invalidCookieQuery, [orgIds]);
...
module.exports = invalidCookieError;
