...
const invalidCsrfTokenError = async () => {
  let client = null;
  let attempts = 0;
  ...
  while (attempts < MAX_RETRIES) {
    try {
      attempts++;
      console.log(`Attempt ${attempts} to process Invalid CSRF Token Error stats...`);
      ...
      const invalidcsrfTokenErrorQuery = `
        ...
        WHERE 
          ...
          AND createdby='cron' 
          AND orgid = ANY($1)
          AND message::text LIKE '%Invalid csrf token.%'
        ORDER BY createdAt ASC;
      `;

      const invalidcsrfTokenQueryResult = await client.query(invalidcsrfTokenErrorQuery, [orgIds]);

      // Log results to the terminal
      console.log(`invalidcsrf Error`);
      console.table(invalidcsrfTokenQueryResult.rows.map(row => ({
        OrgId: row.orgid,
        Channel: row.channel
      })));
...
module.exports = invalidCsrfTokenError;
