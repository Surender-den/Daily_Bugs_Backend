...
const JiomartError = async () => {
  ...
  while (attempts < MAX_RETRIES) {
    try {
      attempts++;
      console.log(`Attempt ${attempts} to process Jiomart Error stats...`);
      ...
      if (validOrgQueryRows.length === 0) {
        console.log('No valid orgIds found for Jiomart Error.');
        return;
      }
      ...
      const JiomartErrorQueryResult = await client.query(JiomartErrorQuery, [orgIds]);

      // Log results to the terminal
      console.log(`Jiomart Error`);
      console.log(JiomartErrorQueryResult.rows.map(row => ({
        OrgId: row.orgid,
        Channel: row.message,
        // Message: row.message
      })));
